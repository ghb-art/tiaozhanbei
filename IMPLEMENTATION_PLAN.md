...
- P0-A4R3已完成并关闭。从v1 HF合并基座重新进行共享Code+NLP蒸馏，
  Math不再单独训练，但每个候选固定加入1000组Math训练回放和128组独立Math验证回放；
  禁止继续训练任务Adapter。
- P0-A4R3只允许两个预注册候选：Candidate 1为Rank 8共享LoRA，Candidate 2为Rank 16
  共享LoRA。二者都必须使用新建的train-only Code/NLP验证集选优，旧42题、96题、170题
  和正式全测逐题结果不得参与超参数调整。
- Code训练数据至少包含3500个真正不同且执行通过的任务，来源最低配额为MBPP 250、
  APPS 2400和CodeContests 750；CodeContests只接受通过隔离stdin/stdout测试的Python 3
  解答。NLP至少包含3000个中文多领域选择题，Teacher生成的翻译、短理由和选项必须通过
  中文质量与原始训练标签双重校验。
- 新验证集固定为Code 256、NLP 256和Math回放128组，训练/验证group完全互斥，并拒绝
  旧验证集及正式测试身份标记。候选只有在Math相对v1保持率不低于95%、Code/NLP均不
  回退且二者平均至少净增1个百分点时，才允许合并和量化。
- 晋级后的量化路线保持不变：训练数据imatrix、Q4_K_M权重、Q8 KV Cache。新的共享模型
  仍须依次通过96题、170题、内存和最终正式门；不得重复运行既有正式全测。
- 与模型能力并行实现工业和交通Fast Path、弱网本地自治/outbox、云边动态调度和安全优先
  冲突仲裁。仿真报告只作为开发证据，最终0.2秒、弱网业务保持率和一致性指标仍须在正式
  双场景部署中复测。
- P0-A4R3实测v1 Math/Code/NLP为`87.50/13.67/63.28%`；Rank 8候选为
  `90.63/5.08/73.05%`，Rank 16候选为`89.06/4.69/73.83%`。两个候选均因Code回退
  被选择门拒绝，生成错误均为0，不得进入量化或96/170题。
- P0-A4R4是当前开放路线。它仍从v1重新训练共享LoRA，不继承失败候选；CodeContests
  只保留执行通过的最短参考解，答案最多768 token，Code推理也预注册为最多768 token。
  MBPP/APPS/CodeContests不复制行，通过显式样本权重使三来源总损失质量相等。
- P0-A4R4重新冻结未见过的Math 128、Code 256（APPS/CodeContests各128）和NLP 256
  训练域验证组。紧凑CodeContests训练答案相对旧数据从P50/P95/最大
  `166/566/1229` token降至`103/427/762` token；新Code验证与P0-A4R3训练和验证
  组重叠均为0。数据总门与Rank 8/16 dry-run均已通过。
