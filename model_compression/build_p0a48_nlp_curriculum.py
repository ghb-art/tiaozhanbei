#!/usr/bin/env python3
"""Build the final failure-heavy, CMMLU-shaped train-only curriculum."""
...
CONFIG = ROOT / "configs/p0a48_nlp_failure_aligned.json"
SOURCE = ROOT / "data/p0a44/nlp_train.jsonl"
MINING_AUDIT = ROOT / "reports/audit/p0a48/mining.json"
MINING_TRACE = ROOT / "reports/audit/p0a48/mining_trace.jsonl"
OUTPUT = ROOT / "data/p0a48/nlp_curriculum.jsonl"
AUDIT = ROOT / "reports/audit/gate_p0a48_curriculum.json"
...
def main() -> int:
    ...
    if mining.get("status") != "passed" or int(mining.get("generation_error_count", -1)) != 0:
        raise RuntimeError("P0-A48 mining audit is not passed")
    ...
    if len(source) != len(source_rows) or set(source) != set(trace):
        raise RuntimeError("P0-A48 source/mining identity mismatch")
    ...
    for sample_id, role, weight in selected:
        ...
        row["sample_id"] = "p0a48/" + hashlib.sha256(sample_id.encode()).hexdigest()[:24]
        row["source"] = "P0-A48-train-only-CMMLU-aligned-failure-repair"
        row["p0a48_role"] = role
        row["training_weight"] = weight
        row["kl_weight"] = float(cfg["kl_weight"])
        row["answer_token_weight"] = float(cfg["answer_token_weight"])
    ...
    audit = {
        "gate": "P0-A48-NLP-CURRICULUM",
        "check_version": "1.0",
        "created_by": "model_compression/build_p0a48_nlp_curriculum.py",
        ...
        "answer_token_weight": float(cfg["answer_token_weight"]),
