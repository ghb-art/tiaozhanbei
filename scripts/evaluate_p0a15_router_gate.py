#!/usr/bin/env python3
"""Run the P0-A15 gate with selected scaled Math LoRA and thinking enabled."""
...
SELECTION = ROOT / "reports/audit/p0a15/math_selection.json"
...
def main() -> int:
    ...
    try:
        ...
        if (
            ...
            or selection.get("selected_model_id") != args.model_id_math
        ):
            raise EvaluationError("P0-A15 selected Math model mismatch")
        ...
        if output != (ROOT / "data/eval/p0a15_router_hf_gate300.jsonl").resolve():
            raise EvaluationError("Unexpected P0-A15 trace path")
        if audit_path != (ROOT / "reports/audit/gate_p0a15_router_hf_gate300_eval.json").resolve():
            raise EvaluationError("Unexpected P0-A15 audit path")
        if output.exists() or audit_path.exists():
            raise EvaluationError("P0-A15 gate artifacts already exist; repeated run refused")
        ...
        for index, row in enumerate(rows, 1):
            ...
            item = {
                "capability_eval_version": "p0a15-gate300-v1",
            ...
            trace.append(item)
            print(
                f"[{index}/300] {domain} thinking={domain == 'math'} "
        ...
        audit = {
            "gate": "P0-A15-ROUTER-GATE300-EVAL",
            "check_version": "1.0",
            "created_by": "scripts/evaluate_p0a15_router_gate.py",
            "created_ts": created_ts,
            "status": "passed" if errors == 0 else "failed",
            "candidate_name": args.candidate_name,
            "served_model_id_by_domain": models,
            "selected_math_scale": selection["selected_scale"],
        ...
        print(f"P0-A15 gate evaluation failed: {exc}", file=sys.stderr)
        return 2
