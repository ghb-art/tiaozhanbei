#!/usr/bin/env python3
"""Run the single P0-A42 aggregate-only 300-item capability test."""
...
OUTPUT = ROOT / "data/eval/p0a42_one_round_gate300.jsonl"
AUDIT = ROOT / "reports/audit/gate_p0a42_one_round_gate300_eval.json"
...
def main() -> int:
    ...
    try:
        if OUTPUT.exists() or AUDIT.exists():
            raise EvaluationError("P0-A42 test artifacts already exist; repeated run refused")
        rows = read_jsonl(MANIFEST)
        counts = Counter(str(row.get("domain", "")) for row in rows)
        if counts != Counter({"math": 100, "code": 100, "nlp": 100}):
            raise EvaluationError(f"Unexpected P0-A42 counts: {counts}")
        requested = {
        ...
        for index, row in enumerate(rows, 1):
            ...
            item = {
                "capability_eval_version": "p0a42-one-round-gate300-v1",
                "created_ts": created_ts,
                "candidate_name": "p0a42-one-round-router-hf",
        ...
        correct_counts = Counter(str(row["domain"]) for row in trace if row["correct"] is True)
        errors = sum(bool(row["generation_error"]) for row in trace)
        accuracy = {domain: correct_counts[domain] / 100 for domain in DOMAINS}
        audit = {
            "gate": "P0-A42-ONE-ROUND-GATE300-EVAL",
            "check_version": "1.0",
            "created_by": "scripts/evaluate_p0a42_gate300.py",
            "created_ts": created_ts,
        ...
        print(f"P0-A42 evaluation failed: {exc}", file=sys.stderr)
        return 2
