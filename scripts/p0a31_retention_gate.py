#!/usr/bin/env python3
"""Compare scaled NLP candidate with the frozen P0-A5 14B NLP100 baseline."""
...
CANDIDATE_AUDIT = ROOT / "reports/audit/p0a31/candidate.json"
CANDIDATE_TRACE = ROOT / "reports/audit/p0a31/candidate_trace.jsonl"
SELECTION = ROOT / "reports/audit/p0a30/nlp_scale_selection.json"
OUTPUT = ROOT / "reports/audit/gate_p0a31_nlp_retention.json"
...
def main() -> int:
    try:
        ...
        if selection.get("status") != "passed" or float(selection.get("selected_scale", 0)) != 0.75:
            raise RuntimeError("P0-A30 selected scale changed")
        ...
        if len(baseline_rows) != 100 or len(candidate_rows) != 100 or set(baseline_rows) != set(candidate_rows):
            raise RuntimeError("P0-A31 sample ids differ from frozen baseline")
        prompt_mismatch = 0
        for row in manifest:
            sample_id = str(row["sample_id"])
            expected_hash = sha256_text(json.dumps(build_messages(row), ensure_ascii=False, sort_keys=True))
        ...
        report = {
            "gate": "P0-A31-NLP100-RETENTION",
            "check_version": "1.0",
            "created_by": "scripts/p0a31_retention_gate.py",
            "created_ts": datetime.now(timezone.utc).isoformat(),
            "status": status,
            "decision": "proceed_to_quantized_nlp_deployment" if recommended_full else "continue_nlp_training",
            "selected_scale": 0.75,
        ...
        print(f"status={status} candidate={candidate_correct}/100 baseline=79/100 retention={retention:.6f}")
        return 0 if recommended_full else 1
    except (OSError, ValueError, KeyError, json.JSONDecodeError, RuntimeError) as exc:
        print(f"P0-A31 retention failed: {exc}", file=sys.stderr)
        return 2
