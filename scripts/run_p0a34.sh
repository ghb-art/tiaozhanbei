...
GPUS="${P0A34_GPUS:-0,1,2,3}"
SERVE_GPU="${P0A34_SERVE_GPU:-0}"
VALIDATION_PORT="${P0A34_VALIDATION_PORT:-18496}"
BASE_DIR="models/checkpoints/p0a4/student-shared-merged"
INIT_ADAPTER="models/checkpoints/p0a32/nlp-continuation/checkpoint-128"
OUTPUT_DIR="models/checkpoints/p0a34/nlp-chinese-exam"
TRAIN_DATA="data/p0a34/train.jsonl"
TRAIN_AUDIT="reports/audit/gate_p0a34_train.json"
AUDIT_ROOT="reports/audit/p0a34"
...
if len(ids)!=4 or len(set(ids))!=4 or not all(item.isdigit() for item in ids):
 raise SystemExit(f"P0-A34 requires four distinct GPU ids: {ids}")
print("P0-A34 GPU group:",','.join(ids))
PY
}
...
data_build() {
  require_status reports/audit/gate_p0a34_teacher_data.json passed
  "$PYTHON_BIN" model_compression/build_p0a34_train.py
  require_status reports/audit/gate_p0a34_train_data.json passed
}

preflight() {
  require_status reports/audit/gate_p0a34_train_data.json passed
  [[ -d "$BASE_DIR" && -s "$INIT_ADAPTER/adapter_model.safetensors" ]] || {
    echo "Missing P0-A34 base or initial adapter" >&2; return 1;
  }
  "$PYTHON_BIN" model_compression/train_p0a6_student.py \
    --model-dir "$BASE_DIR" --train-data "$TRAIN_DATA" --output-dir "$OUTPUT_DIR" \
    --audit reports/audit/gate_p0a34_preflight.json \
  ...
  require_status reports/audit/gate_p0a34_preflight.json dry_run_passed
}
...
train() {
  ...
  if [[ -f "$TRAIN_AUDIT" ]]; then
    ...
    if [[ "$status" == passed ]]; then
      ...
      echo "P0-A34 training already complete."
      return 0
    fi
  fi
...
evaluate_once() {
  ...
  "$PYTHON_BIN" scripts/evaluate_p0a11_domain.py \
    --protocol p0a34 --domain nlp --manifest data/p0a34/nlp_validation.jsonl \
    --endpoint "$endpoint" --model-id "$model_id" --candidate-name "$model_id" \
    --expected-rows 260 --workers "${P0A34_EVAL_WORKERS:-8}" \
...
validation() {
  ...
  "$PYTHON_BIN" scripts/serve_vllm_teachers.py \
    ...
    --quantization none --tensor-parallel-size 1 --served-model-name p0a34-base \
    --lora-module "p0a34-initial=$ROOT/$INIT_ADAPTER" \
    --lora-module "p0a34-nlp-64=$ROOT/$OUTPUT_DIR/checkpoint-64" \
    --lora-module "p0a34-nlp-128=$ROOT/$OUTPUT_DIR/checkpoint-128" \
    --max-model-len 1536 --gpu-memory-utilization 0.80 \
    >logs/p0a34_validation_server.log 2>&1 &
  local server_pid=$!
  cleanup() {
  ...
  wait_endpoint "$server_pid" "$endpoint" p0a34-base,p0a34-initial,p0a34-nlp-64,p0a34-nlp-128
  evaluate_once "$endpoint" p0a34-initial initial_nlp
  evaluate_once "$endpoint" p0a34-nlp-64 nlp_64
  evaluate_once "$endpoint" p0a34-nlp-128 nlp_128
  set +e
  "$PYTHON_BIN" scripts/select_p0a34_nlp.py \
...
status() {
  local path
  for path in reports/audit/gate_p0a34_requests.json \
    reports/audit/gate_p0a34_teacher_data.json reports/audit/gate_p0a34_train_data.json \
    reports/audit/gate_p0a34_preflight.json "$TRAIN_AUDIT" \
    "$AUDIT_ROOT/initial_nlp.json" "$AUDIT_ROOT/nlp_64.json" \
...
case "${1:-help}" in
  ...
  guarded-validation)
    MEMORY_GUARD_THRESHOLD_PERCENT=60 bash scripts/run_with_memory_guard.sh \
      bash scripts/run_p0a34.sh validation
    ;;
  status) status ;;
  structural-check)
    bash -n scripts/run_p0a34.sh
    "$PYTHON_BIN" -m py_compile model_compression/build_p0a34_train.py \
      model_compression/train_p0a6_student.py scripts/evaluate_p0a11_domain.py \
      scripts/select_p0a34_nlp.py
    ;;
  *)
    echo "Usage: bash scripts/run_p0a34.sh <data-build|preflight|train|guarded-validation|status|structural-check>"
    ;;
esac
