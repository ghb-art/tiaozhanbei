#!/usr/bin/env python3
"""Select a P0-A11 Code checkpoint using aggregate-only MBPP evidence."""
...
def load(path: Path) -> dict[str, Any]:
    if not path.is_file() or AUDIT_ROOT not in path.parents:
        raise SelectionError(f"Invalid P0-A18 audit: {path}")
...
def main() -> int:
    ...
    try:
        ...
        require_canonical = bool(validation["candidate_canonical_not_below_base"])
        ...
        for raw in args.candidate:
            ...
            if require_canonical and canonical + 1e-12 < base_canonical:
                failures.append(
                    f"canonical={canonical:.6f}<base={base_canonical:.6f}"
                )
            candidates.append(
                {
                    "step": step,
        ...
        report = {
            "gate": "P0-A18-CODE-TRANSFER-SELECTION",
            "check_version": "1.0",
            "created_by": "scripts/select_p0a18_code.py",
            ...
            "candidate_canonical_not_below_base": require_canonical,
            "candidates": sorted(candidates, key=lambda item: int(item["step"])),
            "selected_step": int(selected["step"]) if selected else None,
            "selected_model_id": (
                f"p0a18-code-{selected['step']}" if selected else None
            ),
            "mbpp_per_item_feedback_read": False,
        ...
        print(f"P0-A18 selection failed: {exc}", file=sys.stderr)
        return 2
