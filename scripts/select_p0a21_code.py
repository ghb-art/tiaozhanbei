#!/usr/bin/env python3
"""Select a P0-A21 checkpoint from aggregate fresh-OpenCode audits."""
...
def load(path: Path) -> dict[str, Any]:
    if not path.is_file() or AUDIT_ROOT not in path.parents:
        raise SelectionError(f"Invalid P0-A21 audit: {path}")
...
def main() -> int:
    ...
    try:
        ...
        report = {
            "gate": "P0-A21-FRESH-CODE-SELECTION",
            "check_version": "1.0",
            "created_by": "scripts/select_p0a21_code.py",
            ...
            "selected_model_id": f"p0a21-code-{selected['step']}" if selected else None,
        ...
        print(f"P0-A21 selection failed: {exc}", file=sys.stderr)
        return 2
