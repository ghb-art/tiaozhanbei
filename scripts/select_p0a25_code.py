#!/usr/bin/env python3
"""Select a failure-repair Code checkpoint from aggregate P0-A25 audits."""
...
def load(path: Path) -> dict[str, Any]:
    if not path.is_file() or AUDIT_ROOT not in path.parents:
        raise SelectionError(f"Invalid P0-A25 audit: {path}")
...
def main() -> int:
    ...
    try:
        ...
        for raw in args.candidate:
            ...
            if gain_over_initial + 1e-12 < 0.01:
                failures.append(
                    f"gain_over_initial={gain_over_initial:.6f}<required=0.010000"
                )
        ...
        report = {
            "gate": "P0-A25-FAILURE-REPAIR-SELECTION",
            "check_version": "1.0",
            "created_by": "scripts/select_p0a25_code.py",
            ...
            "minimum_gain_over_initial_adapter": 0.01,
            ...
            "selected_model_id": f"p0a25-code-{selected['step']}" if selected else None,
        ...
        print(f"P0-A25 selection failed: {exc}", file=sys.stderr)
        return 2
