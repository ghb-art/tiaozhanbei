#!/usr/bin/env python3
"""Select a P0-A32 NLP checkpoint from aggregate train-only validation audits."""
...
def load(path: Path) -> dict[str, Any]:
    if not path.is_file() or AUDIT_ROOT not in path.parents:
        raise SelectionError(f"Invalid P0-A32 audit: {path}")
...
def main() -> int:
    ...
    try:
        ...
        report = {
            "gate": "P0-A32-NLP-CONTINUATION-SELECTION",
            "check_version": "1.0",
            "created_by": "scripts/select_p0a32_nlp.py",
        ...
        print(f"P0-A32 selection failed: {exc}", file=sys.stderr)
        return 2
